...
PCB::PCB(Body body_, void* arg_, uint64* stack_, uint64* context_)
    : wakeupResult(0), waitN(0), stack(stack_), next(nullptr),
      body(body_), arg(arg_), finished(false)
{
...
static void switchToNext(bool requeueCurrent) {
    ...
    if (oldThread)
        ...
        PCB::loadContext(&newThread->context); // new tiny asm routine: just ld ra/sp + ret
